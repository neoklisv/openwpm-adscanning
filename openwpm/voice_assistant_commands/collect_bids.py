...
class BidCollectionCommand(BaseCommand):
    ...
    def execute(
        ...
        try:
            ...
            if len(bids) > 0:
                file_addr = os.path.join(manager_params.data_directory, 'website_bids', str(self.visit_id) + '_' +  str(self.tranco_index) + '_' + method + '_' + self.site.replace('http://www.','') + '.json')
